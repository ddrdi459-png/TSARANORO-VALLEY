(() => {
  ...
  async function syncPublicData(){
    try {
      ...
      const activities=(a.data||[]).map(x=>({id:x.id,name:x.nom,duration:'Selon activité',price:x.prix||'Sur demande',difficulty:'À définir',description:x.description||'',trek:'',equipment:'',photo:x.image_url||'',photos:Array.isArray(x.images)?x.images:(x.image_url?[x.image_url]:[])}));
      const guides=(g.data||[]).map(x=>({id:x.id,name:x.nom,description:x.description||'',phone:'',photo:x.image_url||''}));
      const hotels=(h.data||[]).map(x=>({id:x.id,name:x.nom,description:x.description||'',location:x.adresse||'Vallée de Tsaranoro, Madagascar',phone:x.telephone||'',whatsapp:x.whatsapp||'',price:x.prix||'Sur demande',photo:x.image_url||'',photos:Array.isArray(x.images)?x.images:(x.image_url?[x.image_url]:[]),services:x.services||'',url:x.site_web||''}));
  ...
  function renderMedia(){
    const gallery=document.getElementById('galleryGrid'), videoRoot=document.getElementById('videoLibrary');
  ...
  document.addEventListener('submit', async e => {
    ...
    const name=document.getElementById('name')?.value.trim(), country=document.getElementById('country')?.value.trim(), choice=document.getElementById('choice')?.value.trim(), guide=document.getElementById('guideChoice')?.value.trim(), hotel=document.getElementById('hotelChoice')?.value.trim(), date=document.getElementById('date')?.value||null, people=Number(document.getElementById('people')?.value||1), email=document.getElementById('email')?.value.trim()||null, message=document.getElementById('message')?.value.trim()||'';
    ...
    alert('Réservation enregistrée. WhatsApp va maintenant s’ouvrir.'); e.target.reset();
  }, true);
  syncPublicData(); setInterval(syncPublicData,60000);
})();